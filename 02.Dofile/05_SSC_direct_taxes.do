...
gen double PIT_base_gross = 0
	foreach var in $PIT_base_list {
		replace PIT_base_gross = PIT_base_gross + `var'
	}

gen double PIT_base_gross_deduct = max(0, PIT_base_gross - ${PIT_deduction}) // personal deduction for PIT purpose
...
forvalues i = 1 / $PIT_brackets {
	local j = `i' - 1
		...
		gen double PIT_`i' = 0 																 if PIT_base_gross_deduct <= ${PIT_cutoff_`j'}
...
if $SY_consistency_check == 1 { 
	egen double net_market_income = rowtotal(${market_income} ${SSC} ${direct_taxes})
