...
gen double child_ben = ${child_benefit} * n_elig_child * 12 // Calculate the amount of the benefit. Do not forget to convert monthly paramenetrs to annual values if relevant

gen n_elig_child_orig = min(n_child, 3) 
gen double child_ben_orig = 100 * n_elig_child_orig * 12 // We calculate the original amount of benefits for the baseline separately
...
gen unem_weight = ind_weight
	...
	gen all = 1
	bysort all (rank hh_id p_id): gen double cumulative_unem_weight = sum(unem_weight)	  
...
egen double pre_GMI_income_orig = rowtotal(net_market_income_orig pens_other_trans_orig unem_ben_orig child_ben_orig) 
bysort hh_id: egen double pre_GMI_income_hh_orig = total(pre_GMI_income_orig) // count number of children in the household
gen double pre_GMI_income_pc_orig = pre_GMI_income_hh_orig / hh_size
gen double  GMI_orig = max(0, 1200 * 12 - pre_GMI_income_pc_orig) // This programs cover the income upto the threshold
...
egen double pens_trans_orig = rowtotal(child_ben_orig unem_ben_orig GMI_orig pens_other_trans_orig)
