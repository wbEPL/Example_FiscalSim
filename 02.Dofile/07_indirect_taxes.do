...
gen exp_gross_PY = exp_net_PY  * (1 - exp_form * VAT_rate_PY) * (1 - VAT_ind_eff_PY)

	if $SY_consistency_check == 1 { 
		assert abs(exp_net_SY  * (1 - exp_form * VAT_rate_PY) * (1 - VAT_ind_eff_PY) - exp_gross_SY) < 10 ^ (-10)
	}
	
gen VAT = exp_net_PY - exp_gross_PY

* if we would like to separate the direct and indirect effect this can be done:
gen VAT_dir = exp_net_PY  * exp_form * VAT_rate_PY
gen VAT_ind = VAT - VAT_dir // the direct and indirect effects are rather cumulative than additive, but for simplicity we can assume the additivity
